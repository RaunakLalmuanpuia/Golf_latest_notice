    ...
    public function checkDate(Request $request)
    {
        ...
        switch ($item->name_of_item) {
            ...
            case 'Log Hut (Day & Night)':
                ...
                // Retrieve existing bookings where date range is from beginning to end of all bookings and matching item_id
                // $bookings = OrderedItem::query()
                //     ->whereIn('item_id', $hut)
                //     ->whereHas('orderDetail', function (Builder $builder) {
                //         return $builder->where('payment_status', 'Success');
                //     })
                //     ->get(['date_from', 'date_to', 'quantity']);

                // $existingBookings = $bookings
                //     ->filter(function ($booking) {
                //         return $booking->date_to !== null;
                //     })
                ...
                // // New user's requested booking period

                // // Calculate the total booked quantity within the specified time frame
                // $bookedArray = [];
                // $bookedQuantity = 0;
                // $last30Bookings = array_slice(array_reverse($existingBookings), 0, 30);

                // foreach ($last30Bookings as $booking) {
                //     // Check if the existing booking overlaps with the new booking period
                //     if (
                //         ($newBookingStartDate >= $booking['date_from'] && $newBookingStartDate <= $booking['date_to']) ||
                //         ($newBookingEndDate >= $booking['date_from'] && $newBookingEndDate <= $booking['date_to']) ||
                //         ($newBookingStartDate <= $booking['date_from'] && $newBookingEndDate >= $booking['date_to'])
                //     ) {
                //         // Add the booked quantity if there's an overlap
                //         $bookedArray = $bookedArray + $booking;
                ...
                // $datediff = strtotime($request->date_from) - strtotime($request->date_to);
                // $no_of_days = abs(round($datediff / (60 * 60 * 24))) + 1; // Add 1 to include the end date

                // if ($no_of_days == 0) {
                //     $no_of_days = 1;
                // }
                // // return response()->json(['message' => $availableRooms]);
                // if ($bookedQuantity >= $no_of_huts) {
                //     return response()->json(['message' => 'No more huts available for this date.']);
                // } else {
                //     return response()->json(['message' => 'Huts Available', 'hutsAvailable' => $no_of_huts - $bookedQuantity, 'no_of_days' => $no_of_days]);
                // }
